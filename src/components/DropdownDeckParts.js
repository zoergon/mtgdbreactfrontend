...
const DropdownDeckParts = ({ newId, setNewId, newDeckPartName, setNewDeckPartName, selected, setSelected,  placeHolder, options, isMulti, isSearchable, onChange }) => {
    ...
    const getOptions = () => {
      ...
      return options.filter((option) => option.part.toLowerCase().indexOf(searchValue.toLowerCase()) >= 0)
          ...
          return (
              <div className="dropdown-tags">
                  {selectedValue.map((option) => (
                      <div key={option.partId} className="dropdown-tag-item">
                          {option.part}
    ...
    const removeOption = (option) => {
      return selectedValue.filter((o) => o.partId !== option.partId)
    }
    ...
    const onItemClick = (option) => {
      let newValue
      if (isMulti) {
          if (selectedValue.findIndex((o) => o.partId === option.partId) >= 0) {
    ...
    const isSelected = (option) => {
      if (isMulti) {
          return selectedValue.filter((o) => o.partId === option.partId).length > 0
      }
      if (!selectedValue) {
          return false
      }
      return selectedValue.partId === option.partId      
    }
        ...
        {showMenu && (
            ...
            {isSearchable && (
              <div className="search-box">
                <label>Filter by deck parts:</label>
...
export default DropdownDeckParts